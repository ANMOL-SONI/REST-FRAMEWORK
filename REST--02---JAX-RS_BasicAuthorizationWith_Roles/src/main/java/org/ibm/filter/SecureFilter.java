	...
	public void filter(ContainerRequestContext req) throws IOException {
		...
		if (!method.isAnnotationPresent(PermitAll.class)) {
			if (method.isAnnotationPresent(DenyAll.class)) {
				req.abortWith(Response.status(Status.FORBIDDEN).entity("You can not access this service").build());
				return;
			} // Deny all end
...
//if auth is null or empty-400
				if (list == null || list.isEmpty()) {
					req.abortWith(Response.status(Status.BAD_REQUEST).entity("No Authorization Details found").build());
					return;
				}
...
//split un,pwd
					...
					e.printStackTrace();
					req.abortWith(Response.status(Status.INTERNAL_SERVER_ERROR)
							.entity("Unable to get user/pwd from Auth Header").build());
					return;
				} // catch end
//validateUser
				boolean flag = validateUser(user, pwd);
				if (!flag) { // not a vaild user
